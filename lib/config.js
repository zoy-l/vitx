...
function registerBabel({
  ...
  require('@babel/register')(_objectSpread(_objectSpread({}, bebelConifg), {}, {
    extensions: ['.js', '.ts'],
    only: [(0, _utils.conversion)(_path().default.join(cwd, only))],
    babelrc: false,
    cache: false
  }));
}
...
function _default(cwd) {
  ...
  if (configFile) {
    ...
    const userConfig = isDefault(require(_path().default.join(cwd, configFile)));

    const _schema$validate = _schema.default.validate(userConfig),
