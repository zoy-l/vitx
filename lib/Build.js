...
class Build {
  ...
  transform(opts) {
    ...
    if (/\.(t|j)sx$/.test(paths)) {
      ...
      if (isBrowser) {
        if (nodeFiles && nodeFiles.includes(_path().default.relative(currentDir, paths))) isBrowser = false;
      } else if (browserFiles && browserFiles.includes(_path().default.relative(currentDir, paths))) isBrowser = true;
    }
  ...
  createStream({
    ...
    return _vinylFs().default.src(src, {
      base: basePath,
      allowEmpty: true
    }).pipe((0, _gulpIf().default)(this.watch, (0, _gulpPlumber().default)(() => {}))).pipe(typeof beforeReadWriteStream === 'function' ? beforeReadWriteStream(_through().default) : _through().default.obj()).pipe((0, _gulpIf().default)(({
      path
    }) => tsConfig.declaration && /\.tsx?$/.test(path) && !path.endsWith('.d.ts'), (0, _gulpTypescript().default)(tsConfig))).pipe((0, _gulpIf().default)(({
      path
    }) => /\.(t|j)sx?$/.test(path) && !path.endsWith('.d.ts'), _through().default.obj((chunk, _enc, callback) => {
      ...
      callback(null, chunk);
    }))).pipe(typeof afterReadWriteStream === 'function' ? afterReadWriteStream(_through().default) : _through().default.obj()).pipe(_vinylFs().default.dest(_path().default.join(dir, output)));
  }
