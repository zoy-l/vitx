...
function _gulpLess() {
  const data = _interopRequireDefault(require("gulp-less"));
...
class Build {
  ...
  getBundleOpts(cwd) {
    ...
    const bundleOpts = (0, _lodash.default)(userConfig, this.rootConfig);
    return bundleOpts;
  }
  ...
  transform(opts) {
    ...
    const esBuild = bundleOpts.esBuild,
          _bundleOpts$moduleTyp = bundleOpts.moduleType,
          moduleType = _bundleOpts$moduleTyp === void 0 ? 'cjs' : _bundleOpts$moduleTyp,
          nodeVersion = bundleOpts.nodeVersion;

    if (esBuild) {
      const target = {
        esm: 'chrome58',
        cjs: `node${nodeVersion !== null && nodeVersion !== void 0 ? nodeVersion : 8}`
      };
      return (0, _esbuild().transformSync)(content, {
        loader: _path().default.extname(paths).slice(1),
    ...
    const babelConfig = (0, _getBabelConifg.default)(bundleOpts, paths);
  ...
  createStream({
    ...
    const _bundleOpts$moduleTyp2 = bundleOpts.moduleType,
          moduleType = _bundleOpts$moduleTyp2 === void 0 ? 'cjs' : _bundleOpts$moduleTyp2,
    ...
    return _vinylFs().default.src(src, {
      base: basePath,
      allowEmpty: true
    }).pipe((0, _gulpIf().default)(() => this.watch, (0, _gulpPlumber().default)(() => {}))).pipe((0, _gulpIf().default)(({
      path
    }) => tsConfig.declaration && /\.ts$/.test(path) && !path.endsWith('.d.ts'), (0, _gulpTypescript().default)(tsConfig))).pipe((0, _gulpIf().default)(({
      path
    }) => !!lessOptions && /\.less$/.test(path), (0, _gulpLess().default)(lessOptions))).pipe((0, _gulpIf().default)(({
      path
    }) => /\.(t|j)s?$/.test(path) && !path.endsWith('.d.ts'), _through().default.obj((chunk, _enc, callback) => {
