...
class Build {
  ...
  createStream({
    ...
    return _vinylFs().default.src(src, {
      ...
      this.logInfo({
        pkg,
        msg: `➜${_chalk().default.yellow(` [${esBuild ? 'esBuild' : 'babel'}]:`)} for ${_chalk().default.blue(`${output}${chunk.path.replace(basePath, '')}`)}`
      });
      ...
      if (!file.path.endsWith('.d.ts')) {
        this.logInfo({
          pkg,
          msg: `➜${_chalk().default.yellow(' [Copys]:')} for ${_chalk().default.blue(`${output}${file.path.replace(basePath, '')}`)}`
        });
      }
  ...
  compile(dir, pkg) {
    ...
    this.logInfo({
      pkg,
      msg: _chalk().default.red(`➜ [Target]: ${modeType[bundleOpts.moduleType]}`)
    });
