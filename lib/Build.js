...
class Build {
  ...
  createStream({
    ...
    return _vinylFs().default.src(src, {
      base: basePath,
      allowEmpty: true
    }).pipe((0, _gulpIf().default)(!!sourceMaps, _gulpSourcemaps().default.init())).pipe(typeof beforeReadWriteStream === 'function' ? beforeReadWriteStream({
      through: _through().default,
      insert: _gulpInsert().default,
      gulpIf: _gulpIf().default
    }) : _through().default.obj()).pipe((0, _gulpIf().default)(this.watch, (0, _gulpPlumber().default)(() => {}))).pipe(_gulpInsert().default.transform((contents, file) => {
      ...
      return contents;
    })).pipe((0, _gulpIf().default)(file => tsConfig.compilerOptions.declaration && this.isTransform(/\.tsx?$/, file.path), (0, _gulpTypescript().default)(tsConfig.compilerOptions))).pipe(typeof mountedReadWriteStream === 'function' ? mountedReadWriteStream({
      through: _through().default,
      insert: _gulpInsert().default,
      gulpIf: _gulpIf().default
    }) : _through().default.obj()).pipe((0, _gulpIf().default)(file => this.isTransform(/\.(t|j)sx?$/, file.path), _through().default.obj(function (chunk, _enc, callback) {
      var _that$customPrefix;
      ...
      that.logInfo({
        pkg,
        msg: `➜${_chalk().default.yellow(` [${(_that$customPrefix = that.customPrefix) !== null && _that$customPrefix !== void 0 ? _that$customPrefix : esBuild ? 'esBuild' : 'babel'}]:`)} for ${_chalk().default.blue(`${output}${chunk.path.replace(basePath, '')}`)}`
      });
      ...
      if (!file.path.endsWith('.d.ts')) {
        ...
        this.logInfo({
          pkg,
          msg: `➜ ${_chalk().default.yellow(`[${(_this$customPrefix = this.customPrefix) !== null && _this$customPrefix !== void 0 ? _this$customPrefix : 'Copys'}]:`)} for ${_chalk().default.blue(`${output}${file.path.replace(basePath, '')}`)}`
        });
      }
      ...
      insert: _gulpInsert().default,
      gulpIf: _gulpIf().default
    }) : _through().default.obj()).pipe((0, _gulpIf().default)(file => !!sourceMaps && this.isTransform(/\.jsx?$/, file.path), sourceMaps !== true ? _gulpSourcemaps().default.write() : _gulpSourcemaps().default.write('.', {
  ...
  compile(dir, pkg) {
    ...
    this.logInfo({
      pkg,
      msg: _chalk().default.red(`➜ [Target]: ${(_this$customPrefix2 = this.customPrefix) !== null && _this$customPrefix2 !== void 0 ? _this$customPrefix2 : modeType[bundleOpts.moduleType]}`)
    });
      ...
      createStream(patterns).on('end', () => {
        if (this.watch) {
          ...
          watcher.on('all', (event, fullPath) => {
            ...
            this.logInfo({
              msg: `${(0, _utils.eventColor)(event)} ${(0, _slash().default)(_path().default.join(srcPath, relPath)).replace(`${this.cwd}/`, '')}`
            });
