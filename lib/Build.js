...
function _gulpPathAlias() {
  const data = _interopRequireDefault(require("gulp-path-alias"));

  _gulpPathAlias = function _gulpPathAlias() {
    return data;
  };
...
class Build {
  constructor(options) {
    ...
    this.isLerna = _fs().default.existsSync(_path().default.join(options.cwd, 'lerna.json'));
  }
  ...
  getBundleOpts(cwd) {
    const userConfig = (0, _config.default)(cwd);
    const bundleOpts = this.addDefaultConfigValue((0, _lodash.default)(this.rootConfig, userConfig));
    return bundleOpts;
  }

  transform(opts) {
    var _babelTransformSync;
    ...
    if (esBuild) {
      ...
      return (0, _esbuild().transformSync)(content, esBuildConfig).code;
    }

    const babelConfig = (0, _getBabelConifg.default)(bundleOpts, isBrowser);
    return (_babelTransformSync = (0, _core().transformSync)(content, _objectSpread(_objectSpread({}, babelConfig), {}, {
      filename: paths,
      configFile: false
    }))) === null || _babelTransformSync === void 0 ? void 0 : _babelTransformSync.code;
  }
  ...
  createStream({
    ...
    const moduleType = bundleOpts.moduleType,
    ...
    return _vinylFs().default.src(src, {
      base: basePath,
      allowEmpty: true
    }).pipe((0, _gulpIf().default)(this.watch, (0, _gulpPlumber().default)(() => {}))).pipe((0, _gulpPathAlias().default)({
      paths
    })).pipe(typeof beforeReadWriteStream === 'function' ? beforeReadWriteStream(_through().default, _gulpInsert().default) : _through().default.obj()).pipe((0, _gulpIf().default)(file => tsConfig.compilerOptions.declaration && this.isTransform(/\.tsx?$/, file.path), (0, _gulpTypescript().default)(tsConfig.compilerOptions))).pipe((0, _gulpIf().default)(file => this.isTransform(/\.(t|j)sx?$/, file.path), _through().default.obj((chunk, _enc, callback) => {
      chunk.contents = Buffer.from(this.transform({
        content: chunk.contents,
        paths: (0, _utils.conversion)(chunk.path),
        bundleOpts,
        currentDir: dir
      }));
      this.logInfo({
      ...
      callback(null, chunk);
    }))).pipe(typeof afterReadWriteStream === 'function' ? afterReadWriteStream(_through().default, _gulpInsert().default) : _through().default.obj()).pipe(_vinylFs().default.dest(_path().default.join(dir, output)));
  }
  ...
  compile(dir, pkg) {
    ...
    this.logInfo({
      pkg,
      msg: _chalk().default.gray(`➜ Clean ${output} directory`)
    });
    ...
    return new Promise(resolve => {
      ...
      createStream(patterns).on('end', () => {
        if (this.watch) {
          this.logInfo({
            pkg,
            msg: _chalk().default.blue(`➜ Start watching ${pkg !== null && pkg !== void 0 ? pkg : (0, _utils.conversion)(srcPath).replace(`${this.cwd}/`, '')} directory...`)
          });
          ...
          watcher.on('all', (event, fullPath) => {
            ...
            this.logInfo({
              msg: `${(0, _utils.eventColor)(event)} ${(0, _utils.conversion)(_path().default.join(srcPath, relPath)).replace(`${this.cwd}/`, '')}`
            });
