...
class Build {
  ...
  transform(opts) {
    ...
    let isBrowser = target === 'browser'; // if (/\.(t|j)sx?$/.test(paths)) {
    //   isBrowser = true
    // } else {
    //   if (isBrowser) {
    ...
    //   }
    // }
    // console.log(nodeFiles, path.relative(currentDir, paths))
  ...
  createStream({
    ...
    return _vinylFs().default.src(src, {
      base: basePath,
      allowEmpty: true
    }).pipe((0, _gulpIf().default)(this.watch, (0, _gulpPlumber().default)(() => {}))).pipe(typeof beforeReadWriteStream === 'function' ? beforeReadWriteStream(_through().default) : _through().default.obj()).pipe((0, _gulpIf().default)(file => tsConfig.declaration && this.isTransform(/\.tsx?$/, file.path), (0, _gulpTypescript().default)(tsConfig))).pipe((0, _gulpIf().default)(file => this.isTransform(/\.(t|j)sx?$/, file.path), _through().default.obj((chunk, _enc, callback) => {
