...
class Build {
  ...
  createStream({
    ...
    return _vinylFs().default.src(src, {
      base: basePath,
      allowEmpty: true
    }).pipe((0, _gulpIf().default)(this.watch, (0, _gulpPlumber().default)(() => {}))).pipe(this.applyHook(beforeReadWriteStream, {
      ...
      return contents;
    })).pipe((0, _gulpIf().default)(file => tsConfig.compilerOptions.declaration && this.isTransform(/\.tsx?$/, file.path), (0, _gulpTypescript().default)(tsConfig.compilerOptions))).pipe((0, _gulpIf().default)(file => this.isTransform(/\.(t|j)sx?$/, file.path), _through().default.obj((chunk, _enc, callback) => {
