...
class Build {
  ...
  createStream({
    ...
    return _vinylFs().default.src(src, {
      ...
      this.logInfo({
        pkg,
        msg: `${_chalk().default.green(`➜ [${esBuild ? 'esBuild' : 'babel'}]:`)} for ${_chalk().default.blue(`${output}${chunk.path.replace(basePath, '')}`)}`
      });
  ...
  compile(dir, pkg) {
    ...
    this.logInfo({
      pkg,
      msg: [_chalk().default.gray(`➜ [Clean]: ${output} directory`), _chalk().default.red(`➜ [Target]: ${modeType[bundleOpts.moduleType]}`)].join('\n')
    });
